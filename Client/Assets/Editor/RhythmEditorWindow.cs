...
public class RhythmEditorWindow : EditorWindow
{
    ...
    private bool DrawBlockUI(RhythmBlock block, int index)
    {
        ...
        {
            if (DrawBassNoteUI(block.BassPattern[k], k, block.LengthMeasures))
                bassToRemove = k;
        }
        if (bassToRemove != -1)
        {
    ...
    private bool DrawBassNoteUI(BassNote note, int index, int maxMeasures)
    {
        bool isRemoved = false;
    ...
    private void ExportToServer()
    {
        ...
        ShowNotification(new GUIContent("서버 폴더에 저장 완료! ✔"));
    }
