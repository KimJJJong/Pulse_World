...
- [AppConfig.asset](../Client/Assets/0.MainProject/Resources/AppConfig.asset)
...
- [Server/ApiServer/appsettings.json](../Server/ApiServer/appsettings.json)
...
- 실제 키는 `appsettings.json`에 직접 박지 말고 사용자 비밀 저장소나 환경 변수로 주입

### 3. GameServer -> ApiServer 연결

파일:

- [Server/GameServer/appsettings.json](../Server/GameServer/appsettings.json)
...
"ApiServer": {
  ...
  "SystemApiKey": "CHANGE_ME_SYSTEM_API_KEY"
}
```

이 값은 아래와 일치해야 한다.

- [Server/ApiServer/appsettings.json](../Server/ApiServer/appsettings.json)의 `SystemApiKey`
...
- [Client/steam_appid.txt](../Client/steam_appid.txt)
...
```powershell
dotnet run --project ApiServer/ApiServer.csproj --launch-profile http
```
...
```powershell
dotnet run --project GameServer/GameServer.csproj -- --role Town
```
...
```powershell
dotnet run --project GameServer/GameServer.csproj -- --role Game
```
