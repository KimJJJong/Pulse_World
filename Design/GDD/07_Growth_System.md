...
### 4-1. 맥석 강화 / 합성 (Crystal Reforging)
맥석은 플레이어의 액티브 스킬 그 자체이므로, 이를 육성하는 것이 전투력 상승의 80%를 차지합니다.
*   **제련 (Tier Up):** 하급 맥석 N개 + 맥류 가루(Pulse Dust)를 소모하여, 맥석의 티어를 올려 고정 수치(예: 쿨타임 -1 감소 -> 쿨타임 -2 감소)를 강화합니다.
*   **속성 부여:** 거대한 보스 전리품을 촉매제로 사용하여, 기존 맥석에 [화염 도트 피해 추가] 같은 부가적인 유틸 속성을 인챈트할 수 있습니다.

### 4-2. 무기 한계 돌파 (Awakening) - *선택적 고려*
*   동일한 장비를 2개 이상 제작했을 경우, 하나를 재료로 삼아 메인 장비의 한계 스탯(Max Level)을 확장하는 보편적인 잠재력 돌파 시스템을 지원하여 동일 도면에 대한 지속적인 소모처를 마련합니다.
