...
public sealed class PatternRunner
{
                ...
                case ActionType.MoveStepToward:
                    {
                        ...
                        if (target == null)
                        {
                            Console.WriteLine("[ScheduleTimeline] (target == null)");
                            break;
                        }
                ...
                case ActionType.Attack:
                    {
                        ...
                        // Attack area confirm
                        var frozenCells = ComputeAreaCells(originPoint, act.Area, self: m, target: target);

                        // executeBeat에 맞춰 레지스트리에 저장 (판정이 여기로 감)
                        ...
                        if (teleBeats > 0)
                        {
                            ...
                            if (teleBeat >= baseBeat)
                            {
                                ...
                                //last = Math.Max(last, teleBeat);
                            }
                        }

                        // 실제 공격 예약(텔레그래프 중심과 동일)
                        var cmd = new PlayerActionCmd
                        {
                            ActorId = m.Id,
    ...
    private List<GridPos> ComputeAreaCells( GridPos origin, AreaDef area, MapEntity self, MapEntity target )
    {
        return area.Shape switch
        {
            TelegraphShape.Cells => new List<GridPos>(area.Cells),
            TelegraphShape.Diamond => BuildDiamond(origin, radius: area.ParamA),
            TelegraphShape.Rect => BuildRect(origin, width: area.ParamA, height: area.ParamB),
            TelegraphShape.Line => BuildLine(origin, self, target, length: area.ParamA),
            _ => BuildDiamond(origin, 1)
        };
    }
