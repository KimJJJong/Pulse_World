...
using System.Threading;
    ...
    class SessionManager
    {
        ...
        public T Generate<T>() where T : PacketSession, new()
        {
            lock (_lock)
            {
                ...
                // [ping-fix] Console.WriteLine → LogManager (비동기 큐, 파일 버퍼링)
                LogManager.Instance.LogInfo("SessionManager",
                    $"Connected sessionId={sessionId} type={typeof(T).Name}");
    ...
    public IEnumerable<PacketSession> ByMatch(string currWorld)
    => string.IsNullOrEmpty(currWorld)
       ? Array.Empty<PacketSession>()
       : _sessions.Values.Where(s => s.CurrentWorldId == currWorld).ToArray();
