...
public class CS_Loaded : IPacket
{
	...
	public void Read(ArraySegment<byte> segment)
	{
		...
		count += sizeof(ushort);
		count += sizeof(ushort);
		ushort matcdhIdLen = BitConverter.ToUInt16(s.Slice(count, s.Length - count));
		count += sizeof(ushort);
		this.matcdhId = Encoding.Unicode.GetString(s.Slice(count, matcdhIdLen));
		count += matcdhIdLen;
		ushort uidLen = BitConverter.ToUInt16(s.Slice(count, s.Length - count));
		count += sizeof(ushort);
		this.uid = Encoding.Unicode.GetString(s.Slice(count, uidLen));
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(ushort);
		ushort matcdhIdLen = (ushort)Encoding.Unicode.GetBytes(this.matcdhId, 0, this.matcdhId.Length, segment.Array, segment.Offset + count + sizeof(ushort));
		success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), matcdhIdLen);
		count += sizeof(ushort);
		count += matcdhIdLen;
		ushort uidLen = (ushort)Encoding.Unicode.GetBytes(this.uid, 0, this.uid.Length, segment.Array, segment.Offset + count + sizeof(ushort));
		success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), uidLen);
